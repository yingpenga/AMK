...
class Prompt:
    ...
    def __init__(self, model, tokenizer, text_prompts=None, device='cuda:0'):
        ...
        self.context_length = []
        self.input_ids = []
        self.do_batch_tokenization(self.text_prompts)
    ...
    def do_batch_tokenization(self, text_prompts):
        if text_prompts is None or len(text_prompts) == 0:
            self.input_ids = []
            self.context_length = []
            return

        # batch tokenize for text_prompts
        input_ids_list = []
        context_length_list = []
        ...
        for prompt in text_prompts:
            ...
            context_length_list.append(input_ids.shape[0])

            
        self.input_ids = input_ids_list
        self.context_length = context_length_list